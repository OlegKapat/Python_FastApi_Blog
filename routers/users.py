...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    existing_email = resultEmail.scalars().first()
    if existing_email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_user_page(user_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def update_user(
    ...
    if user_update.username is not None and user_update.username != user.username.lower():
        result = await db.execute(
            select(models.User).where(
